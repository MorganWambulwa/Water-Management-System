...
from .models import WaterSource, IssueReport, RepairLog, WaterVendor, VendorReview
...
class VerificationRequestForm(forms.Form):
    ...
    message = forms.CharField(widget=forms.Textarea(attrs={
        'rows': 4, 
        'placeholder': 'Please provide details proving this source is legitimate...'
    }))
...
class ContactForm(forms.Form):
    name = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'placeholder': 'Your Name', 'autocomplete': 'name'}))
    email = forms.EmailField(widget=forms.EmailInput(attrs={'placeholder': 'Your Email Address', 'autocomplete': 'email'}))
...
class VendorProfileEditForm(forms.ModelForm):
    """Form to allow vendors to edit their profile (including image)."""
    class Meta:
        model = WaterVendor
        fields = ['business_name', 'phone_number', 'location_name', 'price_per_20l', 'delivery_fee', 'is_open', 'image']
        widgets = {
            ...
            'price_per_20l': forms.NumberInput(attrs={
                'class': 'form-control', 
                'min': '0', 
                'step': '1.00' 
            }),
...
class VendorIssueReportForm(forms.ModelForm):
    """Form for Vendors to request a technician."""
    class Meta:
        model = IssueReport
        fields = ['description', 'priority_level']
        widgets = {
            'description': forms.Textarea(attrs={
                'class': 'form-control', 
                'rows': 4, 
                'placeholder': 'Describe the problem (e.g., Pump making noise, Tank leaking...)'
            }),
